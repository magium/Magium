...
class AbstractCheckoutTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $abstractCheckout = $this->getMockBuilder('Magium\Actions\Checkout\AbstractCheckout')->setMethods(null)->getMock();
        /* @var $abstractCheckout \Magium\Actions\Checkout\AbstractCheckout */
        
        $step = $this->getMock('Magium\Actions\Checkout\Steps\StepInterface');
    ...
    public function testStepWillBeExecuted()
    {
        ...
        $step = $this->getMockBuilder('Magium\Actions\Checkout\Steps\StepInterface')->setMethods(['execute'])->getMock();
    ...
    public function testStepReturningFalseMakesExecutionStop()
    {
        ...
        $step1 = $this->getMockBuilder('Magium\Actions\Checkout\Steps\StepInterface')->setMethods(['execute'])->getMock();
        $step1->expects(self::once())->method('execute')->willReturn(false);
        $step2 = $this->getMockBuilder('Magium\Actions\Checkout\Steps\StepInterface')->setMethods(['execute'])->getMock();
